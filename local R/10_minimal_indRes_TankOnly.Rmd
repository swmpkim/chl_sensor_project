...
mod_sel_df <- bind_rows(mod_sel_out, .id = "Reserve") %>% 
  select(Reserve, model, AICc, R.2, pred_test_error, 
         sensor_rfu, fdom_qsu, turb, 
         season, temp, everything())
knitr::kable(mod_sel_df,
             digits = c(2, 2, 2, 3, 2, 4, 4, 
                        4, 4, 4, 4, 4, 4, 4, 
                        4, 4, 2, 2))

write.csv(mod_sel_df, file = here::here("model_coeffs",
                                        "IndRes_TankOnly.csv"), row.names = FALSE)
```
