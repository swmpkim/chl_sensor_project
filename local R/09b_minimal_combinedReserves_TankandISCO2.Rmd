...
This file is almost the same as `05b_minimal_combinedReserves_TankOnly`, but I did NOT filter to tank only. (Commented out line 33 in this Rmd file and then re-ran). I also had to modify row selection from the 'big_table' of coefficients.  
...
all.sel[3, "R^2"] <- summary(no_fdom_ols_lm)$r.squared
```
...
There are two models within 2 AIC units of each other. In the paper, we'll only present the output from the top one. The differences (as seen above) are in 1) whether season is included in the model, and 2) interactions: specifically FDOM\*turb, and FDOM\*RFU\*turb.  

Also, the "no FDOM" model has much higher AICc (3129 vs. 3048) but only slightly more median prediction error (40 vs 36.5).  
...
```{r}
big_table[c(1, 3), ] %>% 
  select(-fdom_qsu.turb, -fdom_qsu.sensor_rfu,
         -fdom_qsu.sensor_rfu.turb) %>% 
  knitr::kable(digits = c(2, 2, 2, 4, 5, 2, 2, 
                          4, 4, 4, 5, 5, 4, 
                          3, 2, 2))

coeffs <- big_table[c(1, 3), ] %>% 
