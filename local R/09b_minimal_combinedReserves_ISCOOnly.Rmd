...
## Modeling: All Reserves, ISCO Only  
...
# save out to CSV
write.csv(coeffs, file = here::here("model_coeffs",                                     "AllRes_ISCOOnly.csv"), row.names = FALSE)
```
...
# save out to csv
write.csv(dat2, file = here::here("model_predictions", "AllRes_ISCOOnly_preds.csv"), row.names = FALSE)
```
...
ggplot(dat2, aes(x = rownum)) +
  ...
  labs(title = "Chl predictions, Field Measurements Only",
...
ggplot(dat2_long) +
  ...
  facet_wrap(~model) +
  labs(title = "Observed by predicted; Field Only; combined-reserve model",
...
ggplot(diags, aes(x = value, y = model)) +
  ...
  ggtitle("National model; Field data only")

ggsave(here::here("model_coeffs", "diagnostics_multimodels_national_field.png"),
